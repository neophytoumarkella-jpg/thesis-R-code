...
Clean_data <- Clean_data[Clean_data$`Attention Check` == "green", ] 
...
length(unique(Clean_data$`Country of Residence`)) 
length(unique(Clean_data$Birthplace))             
...
# STEP 11: CREATE ANALYTIC DATASET => to see if everyone completed my scales
...
#Country of residence(originally a confouding variable and then dropped)
...
length(unique(Clean_data_med$`Country of Residence`)) 
length(unique(Clean_data_med$Birthplace))            
...
mean(Clean_data_med$political_ideology, na.rm = TRUE)   
sd(Clean_data_med$political_ideology, na.rm = TRUE)   
mean(Clean_data_med$gender_role_beliefs, na.rm = TRUE) 
sd(Clean_data_med$gender_role_beliefs, na.rm = TRUE)   
mean(Clean_data_med$system_justification, na.rm = TRUE)
sd(Clean_data_med$system_justification, na.rm = TRUE)  
mean(Clean_data_med$SWB, na.rm = TRUE)                
sd(Clean_data_med$SWB, na.rm = TRUE)                  
...
# STEP 16: REGRESSION MODELS (unstandardized)
...
# STEP 17: REGRESSION MODELS (standardized, for β coefficients)
...
#STEP 21: ANOVA TYPE III- Education=> education as a single factor
...
#STEP 22: HISTOGRAMS - for distributions
